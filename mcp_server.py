...
def handle_rpc(line):
    try:
        ...
        if method == "tools/list":
            tools = [
                {"name": "lion_step", "description": "Perform single optimization step with Lion optimizer"}
            ]
            ...
            if tname == "lion_step":
                p = lion.step(args["param"], args["grad"], args.get("param_id", 0))
                return json.dumps({"jsonrpc": "2.0", "id": rid, "result": {"new_param": p}})
